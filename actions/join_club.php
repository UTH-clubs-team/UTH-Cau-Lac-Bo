...
try {
    ...
    if ($existing) {
        if ($existing['status'] === 'approved') {
            ...
            echo json_encode(['success' => true, 'message' => 'Yêu cầu tham gia đã được gửi, chờ phê duyệt từ quản trị viên']);
            exit;
        }
    }
    ...
    $msg = getenv('APP_ENV') === 'production' ? 'Lỗi máy chủ' : ('Lỗi máy chủ: ' . $e->getMessage());
